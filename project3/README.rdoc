...
Then suppose we want to make a prediction for 4:15pm Then we look at how the weather was for these past 4-7 days around approximately 4.00pm-4.30pm. We regress on this data set and predict what the weather will be for today 4.15pm

2. If there is not at-least 4 previous days of data available:
Then we only regress on today's observations and predictions.
...
To make our predictions we use the following method: 
