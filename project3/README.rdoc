...
8. Once this initialization script has finished running, our application will be initialized. All the web and json views will be operational. The current data for each weather station will be initialized, and the next 3 hours of predictions will be set to the current observation as that is all the application knows. From then on every 10 minutes, one prediction will be made 3 hours into the future (As that prediction made will get old 10 minutes later). It would be ideal to leave our application running for a while so some actual predictions can be made.
